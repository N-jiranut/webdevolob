...
# model.load_model(f"{name}/tabnet_model.zip")

# Now you can use tabnet_loaded to predict:
# y_pred = tabnet_loaded.predict(X_test)

# model = joblib.load(f"{name}/model.pkl")

# model = load_model(f"{name}/model.h5")
# with open(f"{name}/text.txt", "r", encoding="utf-8") as f:
...
def generate_frames():
    ...
    while True:
        ...
        if label !="":
            ...
            draw.text((310, 240), label, font=font, fill=(255, 0, 0))
